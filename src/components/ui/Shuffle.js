...
// Custom Splitting Logic since SplitText is a premium plugin
// This works for simple character splitting which is what Shuffle needs
class CustomSplitText {
    ...
    split(element) {
        ...
        chars.forEach(char => {
            ...
            if (char === ' ') span.style.minWidth = '0.25em'; // Force min width for spaces
    ...
    useGSAP(
        () => {
            ...
            const build = () => {
                ...
                // Use CustomSplitText instead of GSAPSplitText
                ...
                chars.forEach(ch => {
                    ...
                    const w = Math.ceil(rect.width) + 2; // Add buffer to prevent overlap
                    if (!w) return;
            ...
            const cleanupToStill = () => {
                wrappersRef.current.forEach(w => {
                    // w is the wrapper span with overflow:hidden
                    w.style.overflow = 'visible'; // FIX: Allow descenders to be seen after animation
